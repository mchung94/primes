...
An ERROR will be signaled if LOW is greater than HIGH."
  ...
  (loop for i from 1
        when (>= (cl-prime-maker:get-nth-prime i) low)
        return (coerce (loop for j from i
                             for prime = (cl-prime-maker:get-nth-prime j)
                             while (<= prime high)
                             collect prime)
                       'vector)))
